...
class CondConvVAETorchSolver(TorchSolver):
    def __init__(self, model, params, *args, **kwargs):
        super(CondConvVAETorchSolver, self).__init__(model=model, params=params, *args, *kwargs)

    def train_frameset(self, train_frameset, train_cond_frameset):
        self.model.train()
        ...
        cond_frames = [train_cond_frameset[i] for i in idx]

        for frame, cond_frame in zip(frames, cond_frames):
    ...
    def test_frameset(self, test_frameset, test_cond_frameset):
        self.model.eval()
        loss = 0.0
        ...
        cond_frames = [test_cond_frameset[i] for i in idx]
    ...
    def train_for_epochs_frameset(self,
                                  train_frameset, train_cond_frameset,
                                  test_frameset, test_cond_frameset,
                                  fVerbose=False):
        training_losses = []
        test_losses = []
        ...
        for epoch in range(self.epochs):
            train_losses = self.train_frameset(
                train_frameset=train_frameset, train_cond_frameset=train_cond_frameset
            )
            training_losses.extend(train_losses)

            test_loss = self.test_frameset(test_frameset, test_cond_frameset)
    ...
    def train_frameset_and_plot_losses(self,
        ...
        training_losses, test_losses = self.train_for_epochs_frameset(
            train_frameset=train_frameset, train_cond_frameset=train_cond_frameset,
            test_frameset=test_frameset, test_cond_frameset=test_cond_frameset,
            fVerbose=fVerbose
        )
