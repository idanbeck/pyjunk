...
class SpatialViewCondConvEncoder(nn.Module):
    ...
    def ConstructModel(self):
        ...
        #scale_factor = 3
        # num_div_iterations = np.log(W // self.hidden_spatial_extents)
        # num_div_iterations //= np.log(2.0)
...
class SpatialViewCondConvVAE(Model):
    def __init__(self, input_shape, latent_dim, view_dim=7, cond_dim=3, num_layers=12, num_filters=128, *args, **kwargs):
        self.input_shape = input_shape      # input shape is h, w, c
        self.latent_dim = latent_dim
        self.latent_dim_sqrt = int(math.sqrt(latent_dim))
        self.view_dim = view_dim
        self.num_filters = num_filters
    ...
    def ConstructModel(self):
        ...
        self.encoder = SpatialViewCondConvEncoder(
            self.input_shape,
    ...
    def generate_with_cond_frame(self, cond_frame):
        ...
        torchCondImageBuffer = torchCondImageBuffer.permute(0, 3, 1, 2).to(ptu.GetDevice())

        with torch.no_grad():
            #z = torch.randn(1, self.latent_dim)
            z = torch.zeros(self.latent_dim).unsqueeze(0).to(ptu.GetDevice())
            torchViewBuffer = cond_frame.GetFrameCameraView().unsqueeze(0).to(ptu.GetDevice())
            sample = torch.clamp(
    ...
    def loss_with_frame(self, frameObject, condFrameObject):
        ...
        torchCondImageBuffer = torch.FloatTensor(npCondFrameBuffer)
        torchCondImageBuffer = torchCondImageBuffer.unsqueeze(0).to(ptu.GetDevice())
