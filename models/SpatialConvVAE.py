...
class SpatialConvDecoder(nn.Module):
    ...
    def forward(self, input):
        ...
        out = input.view(-1, self.latent_dim, 1, 1).repeat(1, 1, 64, 64)
...
class SpatialConvVAE(Model):
    def __init__(self, input_shape, latent_dim, *args, **kwargs):
        ...
        self.encoder = SpatialConvEncoder(input_shape, self.latent_dim)
        self.decoder = SpatialConvDecoder(self.latent_dim, input_shape, kernel_size=3)
